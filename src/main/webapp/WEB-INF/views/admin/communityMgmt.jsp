        ...
        <div class="columns">
            <div class="column is-3 ">
                <aside class="menu is-hidden-mobile">
                    <ul class="menu-list"
                        style="text-align: left; height: 400px;color: orange;font-weight: bold; border-right: solid 3px;width: 250px;">
                        <br>
                        ...
                        <div class="container">
                            <table class="table is-hoverable is-fullwidth">
                                ...
                                <c:forEach var="item" items="${list}">
                                    <tr onclick="location.href='${path}/community/getCommunity.do?cno=${item.cno}'"
                                        ...
                                        <td class="has-text-centered" width="80"
                                            style="line-height: 40px">${item.cnt}</td>
                                        <td class="has-text-centered" width="80">
                                            <a href="${path}/admin/communityDelete.do?cno=${item.cno}&page=${curPage}"
                            ...
                            <nav class="pagination is-rounded is-centered mb-6" role="navigation"
                                ...
                                <c:if test="${curPage > 5}">
                                    <a href="${path}/admin/communityMgmt.do?page=${page.blockStartNum - 1}<c:if test="${!empty cate}">&cate=${cate}</c:if><c:if test="${!empty page.keyword}">&type=${page.type}&keyword=${page.keyword}</c:if>"
                                       class="pagination-previous">Previous</a>
                                </c:if>
                                <c:if test="${page.blockLastNum < page.totalPageCount}">
                                    <a href="${path}/admin/communityMgmt.do?page=${page.blockLastNum + 1}<c:if test="${!empty cate}">&cate=${cate}</c:if><c:if test="${!empty page.keyword}">&type=${page.type}&keyword=${page.keyword}</c:if>"
                                    ...
                                    <c:forEach var="i" begin="${page.blockStartNum}" end="${page.blockLastNum}">
                                        <c:choose>
                                            <c:when test="${i == curPage}">
                                                <li>
                                                    <a href="${path}/admin/communityMgmt.do?page=${i}"
